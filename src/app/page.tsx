  ...
  return (
    <main className="min-h-screen bg-[#f7f3ea] text-[#111]">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
        <a href="/" className="text-xl font-semibold tracking-tight">Origin Risk</a>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 pt-20">
        ...
        <p className="mt-8 max-w-3xl text-xl leading-8 text-neutral-700">
          Origin Risk classifies long-term productivity, resilience, and impairment risk
          across timber, agriculture, and water resources.
        </p>
        <div className="mt-10 flex flex-wrap gap-4">
          <a href="/api" className="rounded-full bg-black px-6 py-3 text-white">Explore the API</a>
          <a href="/platform" className="rounded-full border border-black px-6 py-3">View platform</a>
        </div>
      </section>
      ...
      <section className="bg-[#111] px-6 py-24 text-white">
        <div className="mx-auto grid max-w-7xl gap-12 md:grid-cols-2">
          ...
          <p className="text-lg leading-8 text-neutral-300">
            We fuse satellite time series, climate records, operational signals, insurance behavior,
            market intelligence, and local ground evidence into a unified asset-state model.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {assetClasses.map((item) => (
            <a key={item.slug} href={`/${item.slug}`} className="rounded-3xl border border-black/10 bg-white/60 p-8 shadow-sm transition hover:bg-white">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24">
        <h2 className="mb-6 text-4xl font-semibold">Eight-dimensional classification</h2>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {dimensions.map((dimension, i) => (
            <div key={dimension} className="rounded-2xl border border-black/10 bg-white/70 p-5">
